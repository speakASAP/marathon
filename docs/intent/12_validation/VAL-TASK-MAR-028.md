...
| Smoke coverage added | Pending | Run deployed pod `npm run check:journey` and confirm `[PASS] landing-missing-gates-ui` appears before expected `[FAIL] catalog-readiness`. |
| Closed landing shows launch blockers | Pending | Validate `/en/#register` renders `Launch blockers` with readiness class chips while registration remains closed. |
| Sensitive data excluded | Pending | Confirm validation records only public readiness labels, build output, and smoke names. |
