...
| Smoke coverage added | Pending | Run deployed pod `npm run check:journey` and confirm `[PASS] gift-missing-gates-ui` appears before expected `[FAIL] catalog-readiness`. |
| Closed gift page shows launch blockers | Pending | Validate `/gift` renders `Gift launch blockers` with readiness class chips while redemption remains closed. |
