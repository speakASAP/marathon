...
- [x] Assignment peer-report tab renders a no-examples explanation when the random report request returns no answer.
- [x] Empty-state copy directs the participant to submit their own report from `Мой отчет`.
- [x] Assignment report submission is disabled when saved-report status cannot be loaded.
- [x] `npm run check:journey` reports assignment peer-report empty-state and saved-status submit-guard UI coverage.
