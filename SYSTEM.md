...
- P0: The current `speakasap-portal` checkout no longer contains usable legacy Marathon models or exporter code; approved catalog-only source data must be provided by a human/source-of-truth before loading `MarathonStep` content through `npm run load:catalog -- <file> --apply`
